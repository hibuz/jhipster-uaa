...
import com.hibuz.uaa.config.KafkaProperties;
...
class UaaKafkaResourceIT {
    ...
    void setup() {
        ...
        UaaKafkaResource kafkaResource = new UaaKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/uaa-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/uaa-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
